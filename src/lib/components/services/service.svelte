<script lang="ts">
	...
	export let icon;
	export let bg = 'bg-buttons-grey';
	export let fg = 'text-buttons-default';
...
<section class="py-16">
	...
	>
		...
		<div class={reverse ? 'lg:order-2' : ''}>
			<div class="mb-4 flex justify-center gap-3 flex-col">
				<div class="rounded-md p-3 w-fit {bg} {fg}">
					<svelte:component this={icon} class="h-6 w-6" />
				</div>
				<h2 class="text-2xl font-semibold">{title}</h2>
			</div>

			<p class="mb-5 text-text-green">
				{description}
			</p>
			...
			>
				<svg
					...
					/></svg
				>Get Quotes on WhatsApp</Button
			>
		</div>

		<!-- Image -->
		<div class={reverse ? 'lg:order-1' : ''}>
			<img src={image} alt={title} class="w-full rounded-2xl object-cover shadow-lg" />
		</div>
	</div>
</section>
